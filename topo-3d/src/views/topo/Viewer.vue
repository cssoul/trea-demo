    ...
    <main class="viewer-canvas-wrap">
      ...
      <div class="legend">
        ...
        <div class="legend-item">
          <span class="legend-line charge" :style="{ background: legendColors.charge, boxShadow: `0 0 6px ${legendColors.charge}` }"></span>
          <span>充电流动</span>
        </div>
        <div class="legend-item">
          <span class="legend-line discharge" :style="{ background: legendColors.discharge, boxShadow: `0 0 6px ${legendColors.discharge}` }"></span>
          <span>放电流动</span>
        </div>
        <div class="legend-item">
...
// 图例颜色：跟随连线全局配置（编辑器工具栏"连线配置"修改后此处同步变化）
const legendColors = ref({ charge: '#00c853', discharge: '#ffb020' })
...
function initCanvas() {
  canvas.value = new TopoScene(canvasRef.value, { mode: 'viewer' })
  // 图例颜色与连线全局配置保持一致
  legendColors.value = {
    charge: canvas.value.getLinkStyleConfig().chargeColor,
    discharge: canvas.value.getLinkStyleConfig().dischargeColor
  }
...
function loadData() {
  ...
  // 数据携带视角（编辑器保存的角度/缩放）时直接恢复，否则自适应居中
...
.ctrl-btn {
  ...
  backdrop-filter: blur(8px);
  box-shadow: 0 4px 12px rgba(31, 45, 66, 0.1);
}

.ctrl-btn:hover {
  color: var(--accent-cyan);
  border-color: var(--accent-cyan);
  box-shadow: 0 0 10px rgba(1, 239, 182, 0.35);
}
