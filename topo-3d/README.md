...
- 连线两端接入设备**顶部接线端子**；母线 / 直线类元素会把接线点在轴线上**就近投影**，形成自然的分支接线。
- 管道由 `TubeGeometry` 沿**二次贝塞尔曲线**生成（控制点位于中点上方，形成"架空电缆桥"），并按第一版连线类型区分弧度：`curve` 高拱、`straight` 低弧、`orthogonal` 抬升 L 形。
...
| 点击电缆 | 选中连线，右侧面板可切换连线类型（曲线 / 直线 / 正交）与线型、线宽 |
